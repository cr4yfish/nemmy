...
export default function Community({
    ...
    useEffect(() => {
        if(session.siteResponse?.my_user) {
            const subCommunties = session.siteResponse.my_user.follows.map((community) => community.community.id + "@" + new URL(community.community.actor_id).host);
            setIsSubscribed(subCommunties.includes(`${community.community.id}@${new URL(community.community.actor_id).host}`))
        }
    ...
    return (
      <motion.div
        ...
        exit={{ opacity: 0, y: 10 }}
        className="w-full flex flex-row justify-between items-center gap-2 rounded-xl border border-neutral-500 bg-neutral-50 p-4 dark:border-neutral-800 dark:bg-neutral-900"
      >
        <Link
        ...
        <div>
            <Button 
                ...
                variant="faded"
                startContent={!subscribeLoading && <span className="material-symbols-outlined">{isSubscribed ? "remove" : "add"}</span>}>
                { subscribeLoading ?
                    <Spinner size="sm" />
                    :
                    <span className=" max-sm:hidden">{isSubscribed ? "Unsubscribe" : "Subscribe"}</span>
                }
            </Button>
        </div>
      </motion.div>
